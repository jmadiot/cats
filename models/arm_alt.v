...
Definition cc0 := dd ⊔ (ctrl ⊔ (addrpo ⊔ cartesian M W ⊓ po_loc)).
...
Definition model_conditions := uniproc /\ (atomic /\ (thinair /\ (propagation /\ (observation /\ scXX)))).
End Model.

Hint Unfold events R W IW FW B RMW F rf po int ext loc addr data ctrl amo unknown_set unknown_relation M emptyset classes_loc X rmw tag2events emptyset_0 partition tag2instrs po_loc rfe rfi co0 toid fencerel ctrlcfence imply nodetour singlestep LKW generate_orders generate_cos cobase coi coe fr fri fre uniproc atomic dd rdw detour addrpo dmb_st dsb_st dmb dsb isb ctrlisb ci0 ii0 cc0 ic0 ppo WW dmb_st_0 dsb_st_0 strong light fence hb thinair hbstar propbase chapo prop propagation observation xx scXX witness_conditions model_conditions : cat.
...
(* End of translation of model ARM *)
