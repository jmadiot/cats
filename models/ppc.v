...
Definition scperlocation := acyclic (po_loc ⊔ (rf ⊔ (fr ⊔ co))).
...
Definition lwsync_0 := lwsync ⊓ !cartesian W R.
Definition eieio_0 := eieio ⊓ cartesian W W.
...
Definition model_conditions := scperlocation /\ (atomic /\ (thinair /\ (propagation /\ (observation /\ scXX)))).
End Model.

Hint Unfold events R W IW FW B RMW F rf po int ext loc addr data ctrl amo unknown_set unknown_relation M emptyset classes_loc X rmw tag2events emptyset_0 partition tag2instrs po_loc rfe rfi co0 toid fencerel ctrlcfence imply nodetour singlestep LKW generate_orders generate_cos cobase coi coe fr fri fre scperlocation atomic dd rdw detour addrpo sync lwsync eieio isync ctrlisync ci0 ii0 cc0 ic0 ppo lwsync_0 eieio_0 strong light fence hb thinair hbstar propbase chapo prop propagation observation xx scXX witness_conditions model_conditions : cat.
...
(* End of translation of model PPC *)
