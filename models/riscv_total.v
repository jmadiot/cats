...
Definition gmo0 := loc ⊓ [(W ⊓ !FW)] ⋅ top ⋅ [FW] ⊔ (ppo ⊔ rfe).
Variable gmo : relation events.
Definition gmo_0 := gmo ⊔ loc ⊓ [IW] ⋅ top ⋅ [(M ⊓ !IW)].
Definition WR := loc ⊓ [W] ⋅ ((gmo_0 ⊔ po) ⋅ [R]).
Definition rfGMO := WR ⊓ !((loc ⊓ [W] ⋅ gmo_0) ⋅ WR).
Definition RfCons := is_empty (rf ⊓ !rfGMO ⊔ rfGMO ⊓ !rf).
Definition infloc := (gmo_0 ⊓ loc)°.
Definition inflocext := infloc ⊓ ext.
Definition winside := infloc ⋅ (rmw ⋅ inflocext) ⊓ (infloc ⋅ (rf ⋅ (rmw ⋅ inflocext)) ⊓ [W]).
