...
(* Translation of model Risc V total order model *)
...
Definition gmo0 := loc ⊓ cartesian (W ⊓ !FW) FW ⊔ (ppo ⊔ rfe).
Variable gmo : relation events.
Definition gmo_0 := gmo ⊔ loc ⊓ cartesian IW (M ⊓ !IW).
Definition WR := loc ⊓ diagonal W ⋅ ((gmo_0 ⊔ po) ⋅ diagonal R).
Definition rfGMO := WR ⊓ !((loc ⊓ diagonal W ⋅ gmo_0) ⋅ WR).
Definition RfCons := is_empty (rf ⊓ !rfGMO ⊔ rfGMO ⊓ !rf).
Definition infloc := (gmo_0 ⊓ loc)°.
Definition inflocext := infloc ⊓ ext.
Definition winside := infloc ⋅ (rmw ⋅ inflocext) ⊓ (infloc ⋅ (rf ⋅ (rmw ⋅ inflocext)) ⊓ diagonal W).
Definition Atomic := is_empty winside.
Definition witness_conditions := linearisations (M ⊓ !IW) gmo0 gmo.
Definition model_conditions := RfCons /\ Atomic.
End Model.

Hint Unfold events R W IW FW B RMW F rf po int ext loc addr data ctrl amo unknown_set unknown_relation M emptyset classes_loc Acq AcqRel Fence_r_r Fence_r_rw Fence_r_w Fence_rw_r Fence_rw_rw Fence_rw_w Fence_tso Fence_w_r Fence_w_rw Fence_w_w Rel Sc X rmw tag2events emptyset_0 partition tag2instrs po_loc rfe rfi co0 toid fencerel ctrlcfence imply nodetour singlestep LKW fence_r_r fence_r_w fence_r_rw fence_w_r fence_w_w fence_w_rw fence_rw_r fence_rw_w fence_rw_rw fence_tso fence po_loc_no_w rsw AcqRel_0 AQ RL AMO RCsc r1 r2 r3 r4 r5 r6 r7 r8 r9 r10 r11 r12 r13 ppo gmo0 gmo_0 WR rfGMO RfCons infloc inflocext winside Atomic witness_conditions model_conditions : cat.

Definition valid (c : candidate) :=
  exists gmo : relation (events c),
    witness_conditions c gmo /\
    model_conditions c gmo.

(* End of translation of model Risc V total order model *)
